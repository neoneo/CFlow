component DateTimeParameter {
	...
	public void function setValue(required string expression, boolean evaluate = false) {
		...
		var date = ListFirst(arguments.expression, "+-");
		if (IsDate(date)) {
		...
		if (partCount == 1) {
			...
			var sign = FindOneOf("+-", arguments.expression, Len(date)); // start looking for + or - after the date value
			...
			throw(type = "cflow.validation", "Expression '#arguments.expression#' is not a valid date/time expression");
		}
